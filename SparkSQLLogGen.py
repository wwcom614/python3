...
def sparksql_generate_log(count = 10, path="OriginalData.txt"):
    f = open(path, "w+")
    while count >= 1:
        query_log = "{ip}\t-\t-\t{local_time}\t{method}\t{http_status}\t{traffic}\t{url_click}".format(ip=sample_ip(),local_time=sample_time(),method=sample_method(),http_status=sample_status(),traffic=sample_traffic(),url_click=sample_url_click())
        print(query_log)
        f.write(query_log+"\n")
        count = count - 1
    f.close()
